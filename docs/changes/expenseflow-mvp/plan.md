...
- **`POST /api/claims/{id}/decision` race-safety uses an EF Core concurrency token, not the pre-existing unique index alone.** A code review after the initial PR found the original check-then-act guard let two concurrent decisions on the same claim both pass, with only a DB-level unique index on `ClaimDecision.ClaimId` as a backstop. That index does protect a real SQL Server at runtime, but it turned out the EF Core InMemory provider used by `ClaimsEndpointsTestFactory` does not enforce unique indexes at all (verified empirically — duplicate inserts against the same unique-indexed column, even within a single `SaveChangesAsync` call, did not throw), so no automated regression test could exercise that path. `ExpenseClaim.Status` is now also marked `IsConcurrencyToken()` in `ExpenseFlowDbContext`, which EF Core's InMemory provider does enforce (throwing `DbUpdateConcurrencyException` on a stale write), giving `DecideClaim` a race-safe path that's actually testable, in addition to still benefiting from the unique index against a real database. This required a new (empty-bodied, metadata-only) EF Core migration — `20260824145649_AddClaimStatusConcurrencyToken` — since concurrency tokens don't need schema changes but do need to be reflected in the model snapshot.
- **`GET /api/claims/pending` now authorizes each pending claim via the `ManagerOfClaim` policy instead of filtering by `ManagerId` in the DB query.** A code review flagged that the "caller is the employee's manager" rule was implemented three separate ways (`ManagerOfClaimHandler`, plus inline copies in `GetClaimById` and `GetPendingClaims`). Both read endpoints now go through the same `IAuthorizationService`/`ManagerOfClaimHandler` policy DecideClaim already used, so there is exactly one place this security-critical rule lives. The trade-off: `GetPendingClaims` now loads all pending claims platform-wide and authorizes each one individually rather than filtering manager-scoped rows in SQL, which is less efficient at large scale but was judged an acceptable trade for this MVP's size and for closing the "single point of truth" gap the plan's own Risks section calls out as the highest-priority mitigation.
